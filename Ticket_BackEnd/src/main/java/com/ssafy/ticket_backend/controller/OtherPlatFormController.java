...
//            System.out.println("더미 데이터 생성 중 오류: " + e.getMessage());
    ...
    private String generateRandomSeat() {
        String[] seatTypes = {"내야지정석", "외야지정석", "1루응원석", "3루응원석"};
    ...
    private int generatePriceBySeatType(String seatInfo) {
        if (seatInfo.contains("내야")) {
            ...
            return 15000 + (random.nextInt(11) * 1000); // 15000, 16000, ..., 25000
        } else if (seatInfo.contains("1루응원석") || seatInfo.contains("3루응원석")) {
            // 1루응원석, 3루응원석: 15000~20000 (1000원 단위)
            return 15000 + (random.nextInt(6) * 1000); // 15000, 16000, ..., 20000
        } else if (seatInfo.contains("외야")) {
            // 외야: 7000~15000 (1000원 단위)
            return 7000 + (random.nextInt(9) * 1000); // 7000, 8000, ..., 15000
        } else {
            // 기타 좌석에 대한 기본값 또는 예외 처리
            return 15000;
        }
    }
}
